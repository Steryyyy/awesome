...
function client.tiled(k, q)
	...
	function client.object.move_to_screen(self, k)
		if self then
			...
			k = j(k)
			if j(self.screen) ~= k then
				local middle = self.screen.workarea.x + self.screen.workarea.width/2
				if self.x + self.width  > middle then
					self.x = k.workarea.x + (self.x- self.screen.workarea.x) + ((k.workarea.width- self.screen.workarea.width) /(self.x < middle and 2 or 1))
				end
